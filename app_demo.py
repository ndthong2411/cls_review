...
def main():
    ...
    primary_metric = st.sidebar.selectbox(
        "🎯 Focus Metric",
        options=['f1', 'pr_auc', 'roc_auc', 'balanced_accuracy', 'sensitivity', 'specificity'],
        index=0,
        format_func=lambda x: {
            'f1': '🎯 F1 Score (Balance)',
            'pr_auc': '📊 PR-AUC (Precision-Recall)', 
            'roc_auc': '📈 ROC-AUC (Overall)',
    ...
    # Radar for top 5
    metrics = ['f1', 'sensitivity', 'specificity', 'pr_auc', 'roc_auc']
    ...
    with col1:
        ...
        st.dataframe(scaler_impact, use_container_width=True)
    ...
    with col2:
        ...
        st.dataframe(imb_impact, use_container_width=True)
    ...
    with col3:
        ...
        st.dataframe(fs_impact, use_container_width=True)
    ...
    with col1:
        ...
        st.dataframe(top_efficient.round(4), use_container_width=True, hide_index=True)
    ...
    with col2:
        ...
        st.dataframe(top_performance.round(4), use_container_width=True, hide_index=True)
    ...
    with st.expander("🔍 Detailed Analysis & Raw Data", expanded=False):
        ...
        with tab1:
            ...
            display_cols = ['model', 'generation', primary_metric, 'sensitivity', 'specificity', 
                           'f1', 'train_time_sec', 'scaler', 'imbalance', 'feature_selection']
            ...
            st.dataframe(
                sorted_df[display_cols].style.background_gradient(
                    subset=[col for col in [primary_metric, 'sensitivity', 'specificity', 'f1'] if col in sorted_df.columns],
                    cmap='RdYlGn'
                ).format({
                    primary_metric: '{:.4f}',
                    'sensitivity': '{:.4f}',
